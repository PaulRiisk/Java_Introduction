...
public class Bank {
    ...
    public boolean withdrawAmount(int amount, Person person) {
        if(getAccountByPerson(person).getBalance() - amount <= -MAX_WITHDRAW) {
            System.out.println("Withdrawing failed. Over the limit.");
            return false;
        }
    ...
    private boolean canBankAccount(Person person, int initialDeposit) {
        ...
        if(!person.isOlderThan(16)) {
            System.out.println(person.getFullName() + " is not old enough to open bank account.");
        } else {
            bankOpened = true;
        }

        if(initialDeposit < MIN_INITIAL_DEPOSIT) {
            System.out.println("The Deposit of " + initialDeposit + " is less than " + MIN_INITIAL_DEPOSIT + ".");
        } else {
            bankOpened = true;
        }
        return bankOpened;
